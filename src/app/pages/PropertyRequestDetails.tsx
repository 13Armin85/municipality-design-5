...
const mockFiles: PropertyFileData[] = [
  { id: "۱-۵۸-۰-۵۹-۵-۷۰۱-۱", owner: "الهام اسدی", kind: "صنفی", fields: ["1","58","0","59","5","701","1"], requests: [{code:"RQ-1405-112",title:"درخواست مفاصاحساب",status:"در حال بررسی",date:"1405/02/21"}], details: [{label:"آخرین مرحله",value:"ارجاع به کارشناس"},{label:"نوع درخواست",value:"مفاصاحساب صنفی"},{label:"شناسه پیگیری",value:"TRK-889134"},{label:"مهلت پاسخ",value:"۳ روز کاری"}] },
  { id: "۳-۲۱-۹-۴۴-۲-۱۲-۰", owner: "محمد رضایی", kind: "آپارتمان", fields: ["3","21","9","44","2","12","0"], requests: [{code:"RQ-1405-027",title:"اصلاح اطلاعات ملک",status:"تکمیل مدارک",date:"1405/01/11"}], details: [{label:"آخرین مرحله",value:"نیازمند بارگذاری سند"},{label:"نوع درخواست",value:"اصلاح مشخصات"},{label:"شناسه پیگیری",value:"TRK-514220"},{label:"مهلت پاسخ",value:"۲ روز کاری"}] },
];
...
export function PropertyRequestDetails({ isDark, toggleTheme }: Props) {
  ...
  return <div dir="rtl" className="min-h-screen bg-background text-foreground transition-colors duration-300">
    ...
    <main className="section-decor px-3 pb-12 pt-24 md:pb-20 md:pt-28 lg:px-6"><div className="container mx-auto max-w-6xl space-y-6">
      ...
      <div className="grid grid-cols-2 gap-2 p-4 md:grid-cols-8"><button onClick={handleSearch} className="h-11 rounded-xl bg-emerald-600 px-6 text-sm font-bold text-white">جستجو</button>{["منطقه","محله","بلوک","ملک","ساختمان","آپارتمان","صنفی"].map((label,i)=><div key={label} className="relative"><input value={searchValues[i]} readOnly className="h-11 w-full rounded-xl border border-border/70 bg-card px-2 text-center text-sm font-medium"/><span className="absolute -top-2 right-2 bg-card px-1 text-[9px] text-muted-foreground">{label}</span></div>)}</div></motion.article>
